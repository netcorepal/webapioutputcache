...
{
    /// <summary>
    /// webapi output cache attribute
    /// </summary>
    public sealed class ApiOutputCacheAttribute : ActionFilterAttribute
    {
        /// <summary>
        /// 缓存时间，单位：秒
        /// </summary>
        ...
        {
            this.Provider = ApiOutputCacheConfig.DefaultProvider;
        }
        ...
        string GetCacheKey(HttpActionContext actionContext)
        {
            ...
            if (!string.IsNullOrEmpty(VaryByCustom))
            {
                ...
                {
                    if (ApiOutputCacheConfig.GlobalVaryByCustomFunc != null)
                    {
                        sb.Append(item.ToLower());
                        sb.Append(ApiOutputCacheConfig.GlobalVaryByCustomFunc(actionContext.Request, item));
                    }
                    else
                    {
